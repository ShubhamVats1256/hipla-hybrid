...
import com.hipla.sentinelvms.sentinelKt.foodModule.network.request.OrderRequestMeetingRoom
...
class OrderPlaceMeetingRoomViewModel constructor(private val repository: CommonRepository) : ViewModel() {
    ...
    fun sendOrderDetail(
        apiKey: String,
        orderRequest: OrderRequestMeetingRoom
    ) {
        loading.value = true
        val response = repository.sendOrderMetingDetail(apiKey, orderRequest)
